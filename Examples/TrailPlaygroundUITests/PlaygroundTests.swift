...
final class PlaygroundTests: XCTestCase {
    @MainActor func testLoadingArcMorphAndRapidMapGestures() throws {
        let app=XCUIApplication(); app.launchArguments=["--trail-journey","cab"]; app.launch()
        let play=app.buttons["journeyPlayPause"]
        XCTAssertTrue(play.waitForExistence(timeout: 15))
        if play.label=="Pause" { play.tap() }
        let load=app.buttons["journeyLoadRoute"]
        if !load.isHittable { app.swipeUp() }
        load.tap()
        let phase=app.staticTexts["journeyRoutePhase"]
        XCTAssertTrue(phase.waitForExistence(timeout: 5))
        let ready=NSPredicate(format: "label == %@","Route ready")
        expectation(for: ready,evaluatedWith: phase); waitForExpectations(timeout: 15)
        if play.label=="Pause" { play.tap() }
        app.sliders["journeyProgress"].adjust(toNormalizedSliderPosition: 0.65)
        let map=app.maps.firstMatch
        XCTAssertTrue(map.exists)
        for _ in 0..<3 { map.swipeLeft(velocity: .fast); map.swipeRight(velocity: .fast) }
        XCTAssertEqual(play.label,"Play")
        XCTAssertEqual(app.staticTexts["journeyMapStatus"].label,"Apple Maps · ready")
        let shot=XCTAttachment(screenshot: app.screenshot()); shot.name="Native Apple map after zigzag gestures"; shot.lifetime = .keepAlways; add(shot)
    }
