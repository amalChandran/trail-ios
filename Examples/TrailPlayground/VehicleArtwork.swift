...
@MainActor struct VehicleArtwork {
    ...
    var image: UIImage { Self.images[id]! }
    private static let images: [String:UIImage] = fleet.mapValues { artwork in
        let extent = artwork.size * 1.8
        return UIGraphicsImageRenderer(size: CGSize(width: extent,height: extent)).image { context in
            let path = TrailPath([TrailPoint(extent/2,extent/2+1),TrailPoint(extent/2,extent/2)])
            artwork.draw(in: context.cgContext,pose: path.pose(at: 1,headingWindow: 0)!)
        }
    }
