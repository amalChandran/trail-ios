...
@main struct TrailPlaygroundApp: App {
    var body: some Scene { WindowGroup {
        #if DEBUG
        if let i = ProcessInfo.processInfo.arguments.firstIndex(of: "--trail-journey"), ProcessInfo.processInfo.arguments.count > i+1 {
            JourneyExamples(initialJourney: ProcessInfo.processInfo.arguments[i+1])
        } else { StudioView() }
        #else
        StudioView()
        #endif
