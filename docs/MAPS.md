...
`TrailRouteTransition` is a deterministic core state machine. Kotlin’s `rememberTrailRouteTransition` and Swift’s `TrailRouteTransitionState` bind it to each UI framework.
