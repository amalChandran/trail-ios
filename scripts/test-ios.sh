...
mkdir -p "$TRAIL_ROOT/artifacts" "$TRAIL_ROOT/Examples/build"
TRAIL_RESULT="$TRAIL_ROOT/Examples/build/TrailTests-$(date +%Y%m%d-%H%M%S).xcresult"
set --
if [[ -n "${TRAIL_IOS_TEST_ONLY:-}" ]]; then set -- "-only-testing:$TRAIL_IOS_TEST_ONLY"; fi
xcodebuild -project "$TRAIL_ROOT/Examples/TrailPlayground.xcodeproj" -scheme TrailPlayground \
    -destination "platform=iOS Simulator,id=$TRAIL_SIM" -parallel-testing-enabled NO \
    -derivedDataPath "${TRAIL_IOS_DERIVED_DATA:-$TRAIL_ROOT/Examples/build/native}" -resultBundlePath "$TRAIL_RESULT" \
