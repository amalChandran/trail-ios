...
/// Native MapKit convenience host. Existing SwiftUI maps embed TrailMapContent inside Map { }.
@MainActor public struct TrailMap: View {
    ...
    @State private var geometry: TrailMapGeometry
    public init(route: TrailRoute, playback: TrailPlayback, reducedMotion: Bool = false) {
        ...
        _geometry = State(initialValue: TrailMapGeometry(route))
    }
    ...
    public var body: some View {
        GeometryReader { size in
            Map(position: $position) {
                TrailMapContent(geometry: geometry,playback: playback,unitsPerPoint: unitsPerPoint,reducedMotion: reducedMotion)
            }
            ...
            .trailPlayback(playback,reducedMotion: reducedMotion)
        }
        .onChange(of: route.coordinates) { _,_ in geometry = TrailMapGeometry(route) }
