...
public enum TrailAnimations {
    /// Seamless travelling window for loading arcs; reduced motion presents the full connection.
    public static func loading(duration: Duration = .seconds(1.2)) -> TrailAnimationSpec {
        custom(TrailSampler { time in
            let p = time.progress
            let windows = p < 0.22 ? [TrailWindow(1+p-0.22,1),TrailWindow(0,p)] : [TrailWindow(p-0.22,p)]
            return TrailVisualState(windows: windows,head: p)
