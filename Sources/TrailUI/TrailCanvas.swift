...
@MainActor public struct TrailCanvas: View {
    ...
    @State private var rendererCache = PreparedRendererCache()
    ...
    public var body: some View {
        ...
        GeometryReader { geometry in
            ...
            let renderer = rendererCache.resolve(measured)
            let animating = active && phase == .active && !reduced && playback.isPlaying && measured.length > 0
            Canvas { context, _ in
                context.withCGContext { cg in renderer.draw(effect: player.effect, in: cg) { player.frame(layer: $0, reducedMotion: reduced) } }
            }
...
@MainActor private final class PreparedRendererCache {
    private var identity: UUID?
    private var renderer: TrailPreparedRenderer?
    func resolve(_ path: TrailPath) -> TrailPreparedRenderer {
        if identity != path.identity { identity = path.identity; renderer = TrailPreparedRenderer(path: path) }
        return renderer!
    }
}
...
public extension View {
    /// Drive native map content using one controller. Do not also bind that player to TrailCanvas.
    func trailPlayback(_ playback: TrailPlayback, active: Bool = true, reducedMotion: Bool = false) -> some View {
        modifier(NativePlaybackModifier(playback: playback, active: active, forceReducedMotion: reducedMotion))
    }
}
private struct NativePlaybackModifier: ViewModifier {
...
public extension View {
    func trailPlayback(_ playback: TrailPlayback, active: Bool = true, reducedMotion: Bool = false) -> some View {
        modifier(MacNativePlayback(playback: playback,active: active,reducedMotion: reducedMotion))
    }
}
private struct MacNativePlayback: ViewModifier {
    let playback: TrailPlayback
    let active: Bool, reducedMotion: Bool
    @Environment(\.scenePhase) private var phase
    @Environment(\.accessibilityReduceMotion) private var systemReduced
    func body(content: Content) -> some View {
        let running = active && phase == .active && !reducedMotion && !systemReduced && playback.isPlaying
